...
export default function HeroSection({ user, onShowRegistration, onShowLogin, onSetActiveTab }: HeroSectionProps) {
  return (
    ...
    >
      ...
      >
        <div className="relative w-[130px] h-[80px] sm:w-[160px] sm:h-[100px] transition-all duration-500 hover:scale-125 group cursor-pointer drop-shadow-[0_0_20px_rgba(0,255,255,0.3)]">
          <Image
            src="/images/logo_sesp_premium.png"
            alt="Fundación SESP"
            fill
            className="object-contain opacity-90 group-hover:opacity-100 transition-all duration-500 mix-blend-screen"
          />
        </div>
        <div className="relative w-[130px] h-[80px] sm:w-[160px] sm:h-[100px] transition-all duration-500 hover:scale-125 group cursor-pointer drop-shadow-[0_0_20px_rgba(0,255,255,0.3)]">
          <Image
            src="/images/logo_impacta_premium.png"
      ...
      {!user ? (
        ...
        <motion.div variants={itemVariants} className="flex flex-col gap-3 justify-center mt-6 px-4">
          <Button onClick={onShowRegistration} className="bg-primary text-primary-foreground text-lg py-7 rounded-2xl font-black shadow-xl shadow-primary/20 uppercase w-full max-w-md mx-auto flex items-center justify-center gap-2 animate-pulse-glow">
            <span className="text-xl">⚽</span> <span>FICHAR POR MI EQUIPO</span>
          </Button>
